...
<div class="grid grid-cols-3 gap-4 mt-6">
  ...
  <strong class="text-indigo-700">Phù hợp test lặp lại</strong>
  <div class="text-sm text-gray-500 mt-1">Cùng một luồng nhưng nhiều tổ hợp input</div>
  </div>
</div>

<div v-click class="mt-4 p-3 bg-white border border-gray-200 rounded-lg text-sm text-gray-600">
  Khi dùng CSV/JSON, dữ liệu và test logic được tách riêng. Điều này giúp Postman dễ bảo trì hơn khi chỉ cần cập nhật file dữ liệu thay vì sửa từng request.
</div>

<!--
Data-driven: tách test logic khỏi test data để tăng coverage mà không nhân bản script.
-->
