...
4. **Bearer Token (JWT)**: JSON Web Token là tiêu chuẩn công nghiệp gồm 3 phần `Header.Payload.Signature` phân tách bởi dấu `.`.
   ...
   - _Lưu ý bảo mật_: Cần tránh cuộc tấn công Algorithm Confusion (sửa alg từ RS256 thành HS256 để ký token bằng public key) và lưu trữ an toàn trong `httpOnly` cookie để phòng chống XSS.
...
- _Ví dụ_: Tham số phân trang `limit` có giới hạn từ `1` đến `100`. Các điểm kiểm thử là:
...
- _Ví dụ_: Kiểm thử định dạng `email`:
...
- _Ví dụ_: File CSV chứa các hàng `username`, `password`, `expected_status` được nạp vào Postman Collection Runner để tự động kiểm thử hàng loạt kịch bản đăng nhập (đúng mật khẩu, sai mật khẩu, tài khoản bị khóa, thiếu tham số).
...
- _Ví dụ_: API checkout đơn hàng `POST /api/v1/checkout` với các điều kiện:
  1. Giỏ hàng hợp lệ?
...
Trong bộ collection Product Service của dự án, nhóm tổ chức **29 test cases** cho **5 endpoints** và chia thành **9 folders**, tách riêng Happy Path / Negative để có thể chạy lại bằng Runner hoặc Newman mà không cần sửa script.
...
1. **eShop Product Service (Mã nguồn nội bộ)**:
   ...
   - Tích hợp cơ chế bảo mật xác thực bằng Bearer ISO-8601 timestamp cho toàn bộ endpoints, bao gồm cả `GET`, `POST`, `PUT` và `DELETE`.
...
| :---------- | :------------- | :----------------------------- | :---------------------------------- |
| **GET**     | `/products`    | Có (Bearer ISO-8601 timestamp) | Lấy danh sách sản phẩm              |
| **GET**     | `/product/:id` | Có (Bearer ISO-8601 timestamp) | Lấy thông tin chi tiết một sản phẩm |
| **POST**    | `/products`    | Có (Bearer ISO-8601 timestamp) | Tạo mới một sản phẩm                |
| **PUT**     | `/product/:id` | Có (Bearer ISO-8601 timestamp) | Cập nhật toàn bộ thông tin sản phẩm |
| **DELETE**  | `/product/:id` | Có (Bearer ISO-8601 timestamp) | Xóa một sản phẩm khỏi hệ thống      |

- **Cơ chế xác thực**: Mọi request đều phải chứa Header `Authorization: Bearer ISO-8601 timestamp`. Middleware kiểm tra timestamp không quá 1 giờ và không nằm ở tương lai; request sai định dạng hoặc quá hạn sẽ bị từ chối với `401 Unauthorized`.
...
4. **Phần 4: Provider Verification & Breaking Change Simulation**
   ...
   - **Giả lập Breaking Change**: Sửa đổi cấu trúc response body của Provider (ví dụ: đổi trường `name` thành `title`). Chạy lại Provider CI pipeline để chứng minh hệ thống phát hiện lỗi không tương thích lập tức và chặn deploy (`can-i-deploy`).
...
Ngoài phần nhận xét định tính này, nhóm còn lưu một `AI_Audit_Report.md` riêng để ghi lại prompt, AI output và human review theo đúng hướng dẫn sử dụng AI của khoa. Đây là bằng chứng giúp truy vết quá trình tạo nội dung, đặc biệt khi cần kiểm tra lại vì sao một test case, một matcher hay một đoạn script được đưa vào báo cáo cuối cùng.
